...
public struct _DVConcreteAttributeGraphContext {
    private(set) weak var owner: ViewGraph?
...
extension _DVConcreteAttributeGraphContext {
    public func read<T: _DVConcreteAttribute>(
        ...
        let attribute = actualAttribute(for: attribute)
    ...
    func actualAttribute<T: _DVConcreteAttribute>(for attribute: T) -> T {
        guard let x = (attribute as? (any ViewGraphParticipant)) else {
            assertionFailure()
            
            return attribute
        }

        let inheritableAttributes = try! x._dynamicViewGraphContext.inheritedNodes.lazy.compactMap { (node: ViewGraph.Node) -> (any _DVConcreteAttribute)? in
            guard !node.attributes.isEmpty else {
                return nil
            }
            
            if let attribute: any _DVConcreteAttribute = try node.attributes.values.first(byUnwrapping: {
                let attribute = try $0.__conversion()
                
                let isCovariant = TypeMetadata(
                    type(
                        of: attribute
                    )._DVConcreteAttribute_ValueType
                )._isCovariant(to: TypeMetadata(type(of: attribute)._DVConcreteAttribute_ValueType))
                ...
                return try attribute.__conversion()
            } else {
                return nil
            }
        }
            
        return (inheritableAttributes.first.map({ $0 as! T }) ?? attribute)
    }
    
    @usableFromInline
    func watch<Node: _DVConcreteAttribute>(
        _ attribute: Node,
        in transaction: _DVConcreteAttributeTransaction
...
extension _DVConcreteAttributeGraphContext {
    ...
    public func lookupOverride<Node: _DVConcreteAttribute>(
        ...
        let baseOverride = overrides[_DVConcreteAttributeGraph.Override(attribute: attribute)] ?? overrides[_DVConcreteAttributeGraph.Override(attributeType: Node.self)]
